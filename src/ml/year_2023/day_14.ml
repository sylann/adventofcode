...
let tilt (dir : direction) (grid : Grid.t) : Grid.t =
  ...
  let m = -1 in
  (match dir with
  | North -> xs (+) 0 (fun x -> ys (-) (h-1) (fun y -> _cell ~dy:1 y x); _wall ~dy:1 m x)
  | South -> xs (+) 0 (fun x -> ys (+) 0     (fun y -> _cell ~dy:m y x); _wall ~dy:m h x)
  | West  -> ys (+) 0 (fun y -> xs (-) (w-1) (fun x -> _cell ~dx:1 y x); _wall ~dx:1 y m)
  | East  -> ys (+) 0 (fun y -> xs (+) 0     (fun x -> _cell ~dx:m y x); _wall ~dx:m y w)
  );
  grid
